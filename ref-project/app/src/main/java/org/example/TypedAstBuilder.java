...
public class TypedAstBuilder {
    ...
    public record InternalFunction(
        ...
        public FuncType toWasmFuncType() {
            ...
            var result =
                    this.returnType
                            .map(t -> Arrays.asList(t.toWasmValueType()))
    ...
    public record ExternalFunction(
            int id, String env, String name, Optional<Type> returnType, Optional<Params> argTypes)
            implements Function {
    ...
    public record StructType(String name, ArrayList<StructField> fields) {}
    ...
    Result<Function, Function> enterNewFunction(
            String name, Optional<Type> returnType, Optional<Params> args, boolean export) {
        if (this.currentFunction.isPresent()) {
            this.leaveFunction();
        }

        InternalFunction f =
                new InternalFunction(functionId, returnType, args, export, new ArrayList<>());
        functionId += 1;
        ...
        if (args.isPresent()) {
            args.get().params().stream().forEach(a -> this.addVariable(a.id().name(), a.type()));
        }
    ...
    Result<ExternalFunction, ExternalFunction> addExternalFunction(
            String name, String env, Optional<Params> params, Optional<Type> returnType) {
        ...
        ExternalFunction func =
                new ExternalFunction(this.externalFuncId, env, name, returnType, params);
