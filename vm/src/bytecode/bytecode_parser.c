...
bool bytecode_parse_section_id(Bytecode_Reader* reader, Bytecode_Section_Id* out_id) {
...
bool bytecode_parse_type_id(Bytecode_Reader* reader, Bytecode_Type_Id* out_id) {
	uint8_t id = bytecode_read_byte(reader);	
...
bool bytecode_parse_function_type(Bytecode_Reader* reader, Bytecode_Function_Type* out_function_type) {
	...
	if(param_count != 0) {
		...
		for(int i = 0; i < func_type.param_count; i++) {
			if(!bytecode_parse_type_id(reader, &func_type.param_types[i])) {
				SDL_LogError(1, "Invalid type id: %d", (uint8_t)func_type.param_types[i]);
				return false;
			}
		}
	}
	...
	if(return_count != 0) {
		...
		for(int i = 0; i < func_type.return_count; i++) {
			if(!bytecode_parse_type_id(reader, &func_type.return_types[i])) {
...
bool bytecode_parse_type_section(Arena* arena, Bytecode_Reader* reader, size_t section_size, Bytecode_Type_Section* out_type_section) {
	...
	if(function_type_count > 0) {
		...
		out_type_section->function_types = arena_push_count(arena, Bytecode_Function_Type, function_type_count);
		
		for(int i = 0; i < function_type_count; i++) {
			uint8_t func_type_header = bytecode_read_byte(reader);	
			if(func_type_header != FUNCTYPE_HEADER) {
				SDL_LogError(1, "Malformed function type: Wrong header. Got %d, expected %d", func_type_header, FUNCTYPE_HEADER);
				return false;
			}
			bool func_type_ok = bytecode_parse_function_type(reader, &out_type_section->function_types[i]);
			if(!func_type_ok) {
	...
	if(section_size > 0) {
		switch(out_section->id) {
			...
			case Bytecode_Section_Id_Type:
				...
				bytecode_parse_type_section(arena, reader, section_size, &out_section->data.type_section);	
			break;
		}
	}
	return true;
