...
use crate::parser::{module::{DecodedBytecode, FunctionInfo}, op::Op, reader::Position, types::{Limits, ValueType}};

use super::{ctrl::{CtrlFrame, JumpTable}, error::ValidationError};
...
pub struct Context<'src> {
    bytecode: &'src DecodedBytecode, 
    internal_functions: Vec<FunctionInfo<'src>>, 
    imported_functions: Vec<FunctionInfo<'src>>,
}

impl<'src> Context<'src> {
    pub fn new(bytecode: &'src DecodedBytecode) -> Result<Context<'src>, ValidationError>{
        let internal_functions = bytecode.get_internal_function_data()?; 
        let imported_functions = bytecode.get_imported_functions_data()?;

        Ok(Context { bytecode, internal_functions, imported_functions })
    }

    pub fn get_mem(&self, id: usize) -> Result<(Limits, Position), ValidationError> {
        Ok(self.bytecode.memories.as_ref().ok_or(ValidationError::UnexpectedNoMemories)?.0[id].clone())
    }
...
impl<'src> Validator {
    ...
    pub fn pop_val_expect(&mut self, expected: ValueStackType) -> Result<ValueStackType, ValidationError> {
        self.pop_val()
            .map(|v| if v == expected {Ok(v)} else {Err(ValidationError::UnexpectedValueType { got: v, expected })})?
    }

    pub fn pop_val_expect_val(&mut self, expected: ValueType) -> Result<ValueStackType, ValidationError> {
