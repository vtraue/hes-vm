...
template <>
std::optional<Type_Id> Parser::parse(Reader& reader) {
...
template <>
std::optional<Section_Id> Parser::parse(Reader& reader) {
  uint8_t id = reader.get<uint8_t>();
  return section_id_from_byte(id);
}
template <>
std::optional<Type_Index> Parser::parse(Reader& reader) {
...
template <>
std::optional<Export> Parser::parse(Reader& reader) {
  auto export_name = this->parse<std::string_view>(reader);
  if (!export_name) {
...
template <>
std::optional<Function_Type> Parser::parse(Reader& reader) {
  auto out_param_types = this->parse_vec<Type_Id>(reader);
  if (!out_param_types) {
...
template <>
std::optional<Type_Section> Parser::parse(Reader& reader) {
  ...
  if (function_type_count > 0) {
    ...
    for (uint64_t i = 0; i < function_type_count; i++) {
      ...
      auto func_type = this->parse<Function_Type>(reader);
      if (!func_type) {
...
template <>
std::optional<Export_Section> Parser::parse(Reader& reader) {
  return this->parse_vec<Export>(reader);
}
template <>
std::optional<Locals> Parser::parse(Reader& reader) {
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    auto t = this->parse<Type_Id>(reader);
    if (!t) {
...
template <>
std::optional<Imm::Blocktype> Parser::parse(Reader& reader) {
  ...
  if (blocktype_byte == blocktype_empty_id) {
    ...
    auto block_valtype = this->parse<Type_Id>(reader);
    if (block_valtype) {
...
template <>
std::optional<Code_Section> Parser::parse(Reader& reader) {
  return this->parse_vec<Code>(reader);
}

template <>
std::optional<Expression> Parser::parse(Reader& reader) {
  int depth = 0;
  ...
  while (!code_done) {
    ...
    switch (op) {
      ...
      case Op::Loop:
      case Op::If: {
        auto blocktype = this->parse<Imm::Blocktype>(reader);
        if (!blocktype) {
...
bool Parser::parse_next_section(Reader& reader) {
  auto section_id = this->parse<Section_Id>(reader);
  if (!section_id) {
  ...
  if (section_size > 0) {
    switch (section_id.value()) {
      ...
      case Section_Id::Type: {
        ...
        this->type_section = this->parse<Type_Section>(reader);
      } break;
      case Section_Id::Function: {
        SDL_LogInfo(1, "Reading function section");
        this->function_section = this->parse<Function_Section>(reader);
      } break;
      case Section_Id::Export: {
        SDL_LogInfo(1, "Reading export section");
        this->export_section = this->parse<Export_Section>(reader);
      } break;
      case Section_Id::Code: {
        SDL_LogInfo(1, "Reading code section");
        this->code_section = this->parse<Code_Section>(reader);
      } break;
      default: {
...
template <>
std::optional<Code> Parser::parse(Reader& reader) {
  auto size = reader.get<uint32_t>();
  SDL_LogInfo(1, "Code size in bytes: %d", size);

  auto locals = this->parse<Locals>(reader);
  auto expression = this->parse<Expression>(reader);
  if (!expression) {
    Os::log("Unable to red expression");
    return {};
  }
  Os::log("Code done\n");
  Code out_code{
